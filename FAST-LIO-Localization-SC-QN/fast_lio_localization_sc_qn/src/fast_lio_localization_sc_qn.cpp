    ...
    nh_(n_private)
{
    ...
    // publishers
    odom_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/ori_odom_localization", 10, true);
    path_pub_ = nh_.advertise<nav_msgs::Path>("/ori_path_localization", 10, true);
    corrected_odom_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/corrected_odom_localization", 10, true);
    corrected_path_pub_ = nh_.advertise<nav_msgs::Path>("/corrected_path_localization", 10, true);
    corrected_current_pcd_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/corrected_current_pcd_localization", 10, true);
    map_match_pub_ = nh_.advertise<visualization_msgs::Marker>("/map_match_localization", 10, true);
    realtime_pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("/pose_stamped_localization", 10);
    saved_map_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/saved_map_localization", 10, true);
    debug_src_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/src_localization", 10);
    debug_dst_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/dst_localization", 10);
...
void FastLioLocalizationScQn::publishMapToOdom(const ros::Time& stamp)
{
    ...
    map_to_odom.pose.pose.orientation.x = tf_transform.getRotation().x();
